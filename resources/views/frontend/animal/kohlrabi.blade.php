...
<script>
    ...
    function* multiply_generator_probability(generator, probability) {
        for (const it of generator) {
            yield {
                ...it,
                probability: it.probability * probability
            };
        }
    }
        ...
        decay(rate_decay_min, rate_decay_max) {
            ...
            */
            ...
            for (let x = 0; x < ret.prob.length; x++) {
                ...
                ret.prob[x] = sum(x + rate_decay_min + 1 + ret.value_start, x + rate_decay_max + 1 + ret
                    .value_start);
        ...
        */
        for (var dec_phase_1_len = 2; dec_phase_1_len < 4; dec_phase_1_len++) {
            for (var high_phase_1_len = 0; high_phase_1_len < 7; high_phase_1_len++) {
                for (var high_phase_3_len = 0; high_phase_3_len < (7 - high_phase_1_len - 1 +
                    1); high_phase_3_len++) {
                    yield* multiply_generator_probability(
                        generate_pattern_0_with_lengths(given_prices, high_phase_1_len, dec_phase_1_len, 7 -
                            high_phase_1_len - high_phase_3_len, 5 - dec_phase_1_len, high_phase_3_len),
    ...
    function* generate_pattern_1(given_prices) {
        for (var peak_start = 3; peak_start < 10; peak_start++) {
            yield* multiply_generator_probability(generate_pattern_1_with_peak(given_prices, peak_start), 1 / (10 -
                3));
        }
    }
    ...
    function* generate_pattern_3(given_prices) {
        for (let peak_start = 2; peak_start < 10; peak_start++) {
            yield* multiply_generator_probability(generate_pattern_3_with_peak(given_prices, peak_start), 1 / (10 -
                2));
        }
    }

    function get_transition_probability(previous_pattern) {
        if (typeof previous_pattern === 'undefined' || Number.isNaN(previous_pattern) || previous_pattern === null ||
            previous_pattern < 0 || previous_pattern > 3) {
    ...
    function* generate_all_patterns(sell_prices, previous_pattern) {
        const generate_pattern_fns = [generate_pattern_0, generate_pattern_1, generate_pattern_2,
            generate_pattern_3];
    ...
    function analyze_possibilities(sell_prices, first_buy, previous_pattern) {
        const generated_possibilities = Array.from(generate_possibilities(sell_prices, true, previous_pattern));
